-- ── 043_r11_code_steps_v2.sql ────────────────────────────────────────────────
-- R11-21 : passage maximum de steps en execution:"code".
-- Remplace 042 (qui ne faisait que add_tag/reserve/start).
--
-- Contextes :
--   order_context:"original"  → commande d'origine du client (fetch_order, create)
--   output_context:"parent"   → nouvelle commande de retour créée (create_new_return_order)
--   order_context:"parent"    → commande de retour (set_original_order, add_tag, reserve, start)
--
-- Steps qui restent AI (ne peuvent PAS être code) :
--   question / check / instruction  → pas d'action Booqable
--   search_products, add_new_product_line, log_case, draft_email, send_email
--     → outils LLM uniquement (route.ts), absents du switch workflow-executor.ts
--   add_internal_note → contenu dynamique (liste articles), pas de "note" fixe
--   add_sav_comment   → articles issus de la conversation, pas de kept_product_names
-- ─────────────────────────────────────────────────────────────────────────────
