  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          origin_order:       { type: 'string', description: 'Numéro de l\'order d\'origine (original.number)' },
          origin_order_id:    { type: 'string', description: 'ID Booqable de l\'order d\'origine (original.id)' },
          sav_order_id:       { type: 'string', description: 'ID Booqable de la return_order (return.id)' },
          ...
          metadata: {
            type: 'object',
            description: 'Toutes les infos pertinentes du cas. Inclure systématiquement tous les champs disponibles dans les variables.',
            properties: {
              insurance:              { type: 'boolean', description: 'Client assuré (return.insurance)' },
              caution:                { type: 'boolean', description: 'Caution carte active (return.authorisation_card)' },
              security_deposit:       { type: 'boolean', description: 'Dépôt physique actif (return.security_deposit)' },
              grand_total_euros:      { type: 'number',  description: 'Montant total facturé en € (return.grand_total_euros)' },
              invoice_number:         { type: 'string',  description: 'Numéro de facture (return.invoice_number)' },
              stripe_charged:         { type: 'boolean', description: 'Caution Stripe débitée (capture_stripe_deposit exécuté)' },
              stripe_charge_id:       { type: 'string',  description: 'ID de la charge Stripe (return.stripe_charge_id)' },
              stripe_amount_euros:    { type: 'number',  description: 'Montant capturé sur la caution en € (return.captured_amount / 100)' },
              payment_link_created:   { type: 'boolean', description: 'Lien de paiement créé (create_payment_link exécuté)' },
              checkout_url:           { type: 'string',  description: 'URL du lien de paiement (return.checkout_url)' },
              cas:                    { type: 'string',  description: 'Cas applicable (ex: I D, I ND, NI D, NI ND)' },
            },
          },
        },
