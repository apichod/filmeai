...
const BOOQABLE_TOOLS = [
  ...
  { value: 'set_replacement_price',              label: 'set_replacement_price — demander le prix de remplacement article par article (nécessite apply_replacement_prices)' },
  ...
  { value: 'apply_replacement_prices',           label: 'apply_replacement_prices — appliquer les prix saisis sur chaque ligne Booqable' },
  { value: 'build_delivery_event',               label: 'build_delivery_event — extraire et formater les données livraison (IA → vars)' },
  ...
  { value: 'check_payment_link',                 label: 'check_payment_link — vérifier si le lien de paiement est actif ou expiré' },
  ...
  { value: 'create_delivery_event',              label: 'create_delivery_event — envoyer l\'événement dans Google Calendar (Code)' },
  ...
  { value: 'fetch_lines',                        label: 'fetch_lines — récupérer les lignes (Qty x Nom ID) → kept_product_names' },
  ...
  { value: 'finalize_invoice',                   label: 'finalize_invoice — finaliser la facture (ou retourner la facture existante si déjà finalisée)' },
...
const TOOL_IO: Record<string, ToolIO> = {
  ...
  add_sav_date:            { reads: ['id'],            writes: [] },
  add_original_order:      { reads: ['number'],       writes: [] },   // input_context.number → order_context (return)
  ...
  build_delivery_event:          { reads: ['delivery_options', 'number'], writes: ['event_summary', 'event_start_iso', 'event_end_iso', 'event_location', 'event_description'] },
  create_delivery_event:         { reads: ['event_summary', 'event_start_iso', 'event_end_iso', 'event_location', 'event_description'], writes: ['calendar_event_id', 'calendar_event_link'] },
  ...
  check_payment_link:        { reads: ['id'],                                         writes: ['payment_link_status', 'checkout_url', 'payment_charge_id'] },
  fetch_lines:               { reads: ['id'],                                         writes: ['kept_product_names'] },
...
const TOOL_COMPAT: Record<string, ToolCompat> = {
  ...
  read_customer_notes:             'code',
  read_delivery_options:           'code',
  build_delivery_event:            'ai',
  create_delivery_event:           'code',
  check_deposit:                   'code',
  add_discount_with_input_field:   'code',
  add_discount:                    'code',
  remove_discount:                 'code',
...
const PARAMETERS_HINT: Record<string, string> = {
  ...
  apply_replacement_prices: '{"charge_label": "Prix de remplacement"}',
}
...
const TOOL_DOC: Record<string, string> = {
  ...
  fetch_lines:                       'Récupère les lignes de la commande et les formate sous la forme "Qty x Nom du produit ID" (ID = identifiant stock numérique si disponible). Écrit kept_product_names (une ligne par article, séparées par \\n). Ne nécessite que l\'order_id en vars.',
  check_payment_link:                'Vérifie si le lien de paiement de la commande est encore actif ou expiré. Lit l\'URL depuis le champ custom lien_paiement (ou depuis vars si déjà connu). Écrit payment_link_status (active / expired / missing), checkout_url et payment_charge_id.',
  ...
  finalize_invoice:                  'Finalise la facture de la commande (statut draft → finalized). Si une facture finalisée existe déjà, retourne directement son document_id et invoice_number sans en créer une nouvelle. Écrit document_id et invoice_number.',
  ...
  add_original_order:                'Renseigne le numéro de la commande d\'origine dans le champ order_sav de la commande de retour. Lit number depuis le contexte input.',
  set_replacement_price:             'Demande le prix HT de remplacement article par article (un champ de saisie par ligne). Écrit replacement_lines_json et replacement_prices_raw utilisés par apply_replacement_prices.',
  apply_replacement_prices:          'Lit replacement_prices_raw (prix séparés par virgule) et replacement_lines_json (lignes) pour appeler PUT /api/boomerang/lines/{id} sur chaque article avec charge_label "Prix de remplacement" et price_each_in_cents. Retourne kept_product_names au format Qty x Nom ID.',
  add_sav_date:                      'Inscrit la date du jour dans le champ date_sav de la commande.',
